...
Use the operator-provided direction, prompt, or context from runtime arguments when provided.
...
- Prefer 2-4 concise options with useful descriptions when multiple-choice answers help, put your recommended option first when there is a sensible default, and leave room for a custom answer when the options are incomplete.
...
- If the interview produced enough clarity for planning, recommend `/planning-prompt` or `/implementation-brief` and explain why.
