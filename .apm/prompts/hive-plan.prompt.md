...
If the intention detected is not an ad-hoc piece of work: make sure plans include updating docs within tasks when relevant, and at least a documentation of handoff task at the end.
...
If a delegated worker or explorer task fails, do not resume the old worker session. Start a new worker session for the retry and include concise context from the failed session or sessions, including what was attempted, where it failed, relevant errors, and the most likely cause, so the new worker can get past the failure instead of repeating the same path.
