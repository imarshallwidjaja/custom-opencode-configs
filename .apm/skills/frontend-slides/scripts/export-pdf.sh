...
// Bind loopback only. Construct the page URL from the listen address so the
// packaged skill does not embed a literal local URL.
const listenAddr = await new Promise((resolve, reject) => {
...
await page.goto(`http://${listenAddr.address}:${port}/`, { waitUntil: 'networkidle' });
...
for (let i = 0; i < slideCount; i++) {
  await page.evaluate((index) => {
    const deck = window.deck || window.presentation;
    if (deck && typeof deck.showSlide === 'function') {
      deck.showSlide(index);
      return;
    }
