...
```bash
<this-skill-dir>/bin/run validate.py diagram.drawio
```
...
```bash
<this-skill-dir>/bin/run pyimports.py myproject -o graph.json
<this-skill-dir>/bin/run autolayout.py graph.json -o diagram.drawio
```
...
```bash
<this-skill-dir>/bin/run drawiodiff.py old.drawio new.drawio -o diff.json
<this-skill-dir>/bin/run autolayout.py diff.json -o diff.drawio
```
...
```bash
<this-skill-dir>/bin/run timelapse.py src --importer pyimports --max-frames 12
...
```bash
<this-skill-dir>/bin/run edgeports.py diagram.drawio          # spread stacked edges over each perimeter
```
