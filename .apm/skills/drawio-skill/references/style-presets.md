...
> **Existing diagrams:** these rules apply at generation time. To re-theme a `.drawio` that already exists, run `<this-skill-dir>/bin/run restyle.py diagram.drawio --preset <name>` — it applies the palette (hue-mapped), font, and extras without touching layout or edge routing.
