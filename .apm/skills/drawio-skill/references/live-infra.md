...
<this-skill-dir>/bin/run autolayout.py graph.json -o deployed.drawio
```
...
```bash
docker inspect $(docker ps -q) | <this-skill-dir>/bin/run dockerimports.py - -o graph.json
<this-skill-dir>/bin/run autolayout.py graph.json -o running.drawio
```
...
kubectl get deploy,sts,ds,svc,ing,cm,secret,pvc,hpa -n <ns> -o json \
  | <this-skill-dir>/bin/run k8simports.py - -o graph.json
<this-skill-dir>/bin/run autolayout.py graph.json -o cluster.drawio
```
