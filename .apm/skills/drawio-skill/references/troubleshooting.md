...
| Final `-e` PNG won't open in image viewers / vision APIs | Run `<this-skill-dir>/bin/run repair_png.py <path>`. draw.io CLI emits `-e` PNGs with an 8-byte truncation at IEND. SVG/PDF unaffected. |
...
```bash
URL=$(<this-skill-dir>/bin/run encode_drawio_url.py --edit diagram.drawio)
