...
compatibility: Requires uv on PATH plus the draw.io desktop app CLI (macOS/Linux/Windows). Self-check step requires a vision-enabled model (e.g., Claude Sonnet/Opus); gracefully skipped if unavailable. Optional auto-layout (scripts/autolayout.py) needs Graphviz (dot).
...
## Script runner (required)

Never use `python3` or `pip` for this skill. Python libraries (PyYAML, python-pptx, Pillow) are declared in this skill's `pyproject.toml` and resolved by `uv` into a venv that lives **next to the skill**, not in the user's project. That is what makes it work from any repo.

Invoke every bundled script as:

```bash
<this-skill-dir>/bin/run <script.py> [args...]
```

Examples:

```bash
<this-skill-dir>/bin/run autolayout.py graph.json -o diagram.drawio
terraform show -json | <this-skill-dir>/bin/run tfstate.py - -o graph.json
```

`bin/run` wraps `uv run --project <this-skill-dir>`. `uv` must be on PATH (`https://docs.astral.sh/uv/`). First run may fetch wheels into uv's cache; later runs are local. Do not `pip install` anything for this skill.
...
| `scripts/drawio2pptx.py` | The user wants a **PowerPoint deck / slides from a diagram** — `drawio2pptx.py diagram.drawio -o deck.pptx` puts each page on its own 16:9 slide (page name as title), so a multi-page **C4 model** becomes a ready-to-present deck. Needs python-pptx (provided by this skill's uv project) + the draw.io CLI |
...
`uv` must be on PATH. Bundled scripts go through `<this-skill-dir>/bin/run`; do not call `python3` or `pip`.
...
```bash
<this-skill-dir>/bin/run repair_png.py diagram.drawio.png
```
...
```bash
<this-skill-dir>/bin/run encode_drawio_url.py input.drawio          # read-only viewer
<this-skill-dir>/bin/run encode_drawio_url.py --edit input.drawio    # opens in the editor
```
