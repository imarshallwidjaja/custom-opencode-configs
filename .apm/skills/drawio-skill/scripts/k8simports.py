...
need PyYAML (provided by this skill's uv project; invoke via bin/run). Pass `-` to read a live cluster snapshot
        ...
        if path == "-":                                  # `kubectl get ... -o json | k8simports.py -`
            ...
            try:
                ...
                sys.exit(f"error: {path} is YAML but PyYAML is not installed "
                         "(invoke via bin/run so uv provides PyYAML) — or feed JSON from `kubectl get ... -o json`")
