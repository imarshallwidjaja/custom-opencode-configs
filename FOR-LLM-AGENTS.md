...
- `forager-worker` explicitly auto-loads canonical `verification`; `hive-builder` loads orchestration skills without that auto-load. Planner, swarm, and builder include `background-delegation`. Custom agents use only supported `baseAgent` values from the current Hive contract.
...
- Human-facing prose is governed by a `## Prose Finish Gate` section in the Cursor Rules template and in all four AGENTS profiles. It runs an always-on draft, audit, fix loop. Each of the six Cursor agent definitions carries a short prose contract, and the `subagent-delegation` skill requires handoffs to include it. None of this adds a setup step or prerequisite. `writing-for-humans` is the core prose skill; `stop-slop` and `humanizer` are depth skills loaded only when rewriting text that is already slop-heavy. `stop-design-slop` ships `scripts/contrast_check.py` for computed WCAG contrast ratios; like `audit_ui.py`, it needs only `python3`.
