...
- `scout-researcher` is the sole research role. `plan-reviewer` and `code-reviewer` own their full first-pass scope; their former `-xhigh-reasoning` variants are removed. Choose `forager-capable` up front for hard defined work; reserve `forager-smart` for failed, stalled, incomplete, or substantially unspecified work with no closer specialist. Adversarial passes supplement ordinary review, including after every behavior-changing implementation. Portable profiles exclude personal `ivan-writing` and `impeccable` auto-loads and all non-OpenAI providers.
...
- `forager-worker`'s configured `autoLoadSkills` in this profile start with canonical `verification` then compact `writing-policy`. This profile does not add a verification auto-load to `hive-builder`. Agent Hive may prepend built-in defaults before these configured additions, so this file does not describe the runtime-effective skill list. Planner, swarm, and builder include `background-delegation`. Normal Hive seats receive `writing-policy` through their base `autoLoadSkills`, and custom agents inherit it. `hive-helper` is hardcoded to no auto-loaded skills; hidden summarizers are not normal seats. `forager-documents` and the documentation reviewers add the deeper shared prose stack. Custom agents use only supported `baseAgent` values from the current Hive contract.
...
- Human-facing prose is governed by `writing-policy` plus a `## Prose Finish Gate` section in the Cursor Rules template and in all four AGENTS profiles. The gate is an always-on draft, audit, fix loop. Each of the six Cursor agent definitions requires the child to load and apply `writing-policy`, and the `subagent-delegation` skill requires handoffs to state artifact, audience, voice, and any additional depth or domain writing skills because parent-loaded skills do not propagate. None of this adds a setup step or prerequisite. `writing-for-humans`, `stop-slop`, and `humanizer` remain conditional depth skills. `stop-design-slop` ships `scripts/contrast_check.py` for computed WCAG contrast ratios; like `audit_ui.py`, it needs only `python3`.
...
- `personal-default`: the shared rules plus Ivan's voice skill; select that voice only for prose published, submitted, or sent as Ivan. Internal reports stay neutral unless requested.
- `shared-context-improved`: the shared profile plus strong routing rules for `ast-grep`, `context7`, and optional `cymbal`
...
`profiles/base/agent_hive.json` is the sole Hive config source. It uses only non-fast `openai/gpt-5.6-luna`, `openai/gpt-5.6-sol`, and `openai/gpt-6-astra`. Confirm OpenAI ChatGPT OAuth is available before install.
...
```text
Do you want the richer local context and code-navigation workflow on this machine? That enables local ast-grep and context7 together, adds the matching Agent Hive overlay, and pairs best with the shared-context-improved or personal-context-improved AGENTS profile.
```
...
The vendored Engineering Judgment snapshot is provenance-pinned to oc-arkive 2.3.5. OpenCode receives Engineering Judgment from the installed plugin; do not copy the vendored body into OpenCode AGENTS profiles, `profiles/base/agent_hive.json`, OpenCode agent definitions, commands, or skills. Cursor continues to use that vendored snapshot because Cursor cannot load the plugin prompt directly. Provenance `packageVersion` records source-checkout metadata, not publication. Cursor User Rules apply to Agent Chat, not Inline Edit. Project `.cursor/rules/*.mdc` is a separate opt-in path and this helper must not install it automatically.
...
If the context-improved bundle was enabled, this first run also needs to start the local MCP tooling without errors.
...
- verify every Hive model is exactly `openai/gpt-5.6-luna`, `openai/gpt-5.6-sol`, or `openai/gpt-6-astra`, with no `-fast` suffix or other provider; base `opencode.json` agent overrides remain Luna. Confirm model IDs and configured variants with `opencode models openai --verbose`, then separately verify inference access; listing alone is insufficient
